...
| A1 | **Sound X-aware oracle ELIMINATES hand-verification** | `run_careset_equiv` + `run_mixededge_equiv` (equiv.py): two-build care-mask + reset-settle + declared preconditions + **real-latch half-cycle miter** (circuit8 class: `-nolatches` was destroying the latch → false CEX). 156×5 re-score: **flagged RHG 9→0**, all four flagged tasks machine-proven (`q5b`/`prob095`/`prob149`/`circuit8`), HPR Opus/GPT **0.929**, **0 regressions**; controls stay cex (impossible-overfit + inverted/latch-low/wrong-edge mutants). Tests: `tests/test_sound_oracle.py` (10✓, no xfail). Docs: `docs/A1_SOUND_ORACLE.md` | 1 | 🟢 |
...
| M7 | RLVR training probe runs end-to-end (GPU) | CPU loop-validation (`validate_grpo_local.py`, ~12s) **+ real GPU smoke on an A100** (`runpod/rlvr_launch.sh`): Qwen3-4B LoRA, SFT cold-start + 50-step GRPO on the gameable visible reward, withheld-oracle audits at steps 25/50 → **visible=formal=0.833, RHG=0.0 flat, hacked=0/12** (`results/runpod/rhg_curve_z9ujfawvieorl5.jsonl`). The 50-step smoke shows no induced hacking; the full 500-step multi-seed emergence study is a separate paper | 1 | 🟢 |
