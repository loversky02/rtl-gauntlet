...
three models, we find the na\"ive headline \rhg{} ($6.1\%$) is \emph{entirely an oracle artifact}: a
...
(\S\ref{sec:oracle}).
\item An empirical result across $3$ models $\times\,156$ tasks: \textbf{zero verified genuine
...
weaker models fail far more (\code{fail\_visible} $9$/$36$/$30$ for Opus/DeepSeek/Haiku, $+11$
no-candidate for Haiku) but hack no more. All three residual \rhg{} counter-examples are verified artifacts of the BMC zero-init assumption
(\code{-set-init-zero} forces an invalid state for an async-reset/one-hot FSM---DeepSeek and Haiku
independently produced the \emph{same} \code{q5b} one-hot Mealy FSM); the $5$ \code{dontcare}
cases were likewise hand-verified as genuine output don't-cares, and the detector now requires the
\code{x} to drive an output, so an incidental internal \code{x} cannot mask a real CEX. With $95\%$ Wilson
intervals, verified-genuine \rhg{}$=0$ with an upper bound $\le\!2.5\%$ (Opus) / $\le\!3.1\%$ (DeepSeek) / $\le\!3.2\%$ (Haiku).

\begin{table}[t]\centering\small
\begin{tabular}{@{}lccc@{}}
\toprule
 & Opus 4.8 & DeepSeek & Haiku 4.5 \\
\midrule
honest $+$ bmc & 135 & 117 & 107 \\
\hpr{} ($95\%$ CI) & 0.87 [.80,.91] & 0.75 [.68,.81] & 0.69 [.61,.75] \\
\code{fail\_visible} ($+$ no-cand) & 9 & 36 & 30 ($+11$) \\
verified \rhg{} & 0 ($\le.025$) & 0 ($\le.031$) & 0 ($\le.032$) \\
\bottomrule
\end{tabular}
\caption{Three models $\times\,156$ tasks (hardened oracle; \hpr{} counts honest $+$ BMC-verified, so
Opus $=129{+}6=135$, $135/156=0.87$). All verified \rhg{}$=0$: every flagged CEX is an init/encoding
artifact---DeepSeek's and Haiku's are the \emph{same} \code{q5b} one-hot Mealy FSM under zero-init.
...
\caption{Weakness $\ne$ hacking across three models: Haiku/DeepSeek fail more on the visible tests, \rhg{}$=0$ throughout.}
\label{fig:models}
\end{figure}
...
(2-iter $17$ vs $36$). The repair tail is mostly wasted (honest payoff $41\%$ / $14\%$); an
...
\item No false positives on honest agents; across $3$ models $\times\,156$ fair tasks, \textbf{zero
